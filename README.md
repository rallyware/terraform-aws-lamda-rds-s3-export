...
| Name | Source | Version |
|------|--------|---------|
| <a name="module_bucket"></a> [bucket](#module\_bucket) | cloudposse/s3-bucket/aws | 4.0.0 |
| <a name="module_kms_key"></a> [kms\_key](#module\_kms\_key) | cloudposse/kms-key/aws | 0.12.1 |
| <a name="module_lambda"></a> [lambda](#module\_lambda) | rallyware/lambda-function/aws | 0.2.1 |
...
| <a name="input_cloudwatch_event_rules"></a> [cloudwatch\_event\_rules](#input\_cloudwatch\_event\_rules) | A list of CloudWatch Event Rules to trigger the Lambda function | <pre>list(object(<br>    {<br>      name = string<br>      event_pattern = object(<br>        {<br>          detail_type = list(string)<br>          detail = object(<br>            {<br>              message = list(string)<br>            }<br>          )<br>        }<br>      )<br>    }<br>  ))</pre> | <pre>[<br>  {<br>    "event_pattern": {<br>      "detail": {<br>        "message": [<br>          "Automated snapshot created"<br>        ]<br>      },<br>      "detail_type": [<br>        "RDS DB Snapshot Event"<br>      ]<br>    },<br>    "name": "rds-automated-snapshot-created"<br>  },<br>  {<br>    "event_pattern": {<br>      "detail": {<br>        "message": [<br>          "Automated cluster snapshot created"<br>        ]<br>      },<br>      "detail_type": [<br>        "RDS DB Cluster Snapshot Event"<br>      ]<br>    },<br>    "name": "rds-automated-cluster-snapshot"<br>  }<br>]</pre> | no |
